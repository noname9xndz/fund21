...
namespace smartFunds.Common.Helpers
{
    public class RSAHelper
    {
        ...
        public string Encrypt(string text)
        {
            ...
            int keySize = _publicKeyRsaProvider.KeySize/8;
            int maxLength = keySize - 42;
            int dataLength = bytes.Length;
            int interations = dataLength / maxLength;
            StringBuilder sb = new StringBuilder();
            for (int i = 0; i <= interations; i++)
            {
                byte[] tempBytes = new byte[dataLength - maxLength * i > maxLength ? maxLength
                    : dataLength - maxLength * i];
                Array.Copy(bytes, maxLength * i, tempBytes, 0, tempBytes.Length);
                byte[] encryptedBytes = _publicKeyRsaProvider.Encrypt(tempBytes, RSAEncryptionPadding.Pkcs1);
